...
import {
  ...
  recordSystemEvent,
} from "./systemEvents.service";
import { createUserNotification } from "../utils/notifications";
import { emitToUser, emitToWorkers } from "./socketManager";
...
const SCHEDULED_REQUEST_DURATION_MINUTES = Math.max(
  60,
  Math.min(
    Number(process.env.SCHEDULED_REQUEST_DURATION_MINUTES || 120),
    7 * 60,
  ),
);
...
const reinjectScheduledRequestCandidates = async (
  ...
  if (input.latitude == null || input.longitude == null || !input.idService)
    return;
...
const expireScheduledCounterOffers = async () => {
  ...
  for (const row of rows) {
    ...
    const previousWorkerProfile =
      Number(row.assigned_worker_profile || 0) || null;
    ...
    try {
      ...
      await reinjectScheduledRequestCandidates(connection, {
        ...
        latitude:
          lockRows[0].latitude != null ? Number(lockRows[0].latitude) : null,
        longitude:
          lockRows[0].longitude != null ? Number(lockRows[0].longitude) : null,
      ...
      emitToWorkers("request_updated", { id_request: idRequest });
    } catch (error: any) {
      await connection.rollback();
  ...
  for (const row of rows) {
    ...
    try {
      ...
      if (clientUserId) {
        ...
        emitToUser(clientUserId, "request_updated", { id_request: idRequest });
      }
      if (workerUserId) {
        await createUserNotification({
          userId: workerUserId,
...
const processScheduledBookingMaintenance = async () => {
  const now = Date.now();
  if (now - scheduledMaintenanceLastRun < SCHEDULED_MAINTENANCE_INTERVAL_MS)
    return;
...
const runJobHandler = async (row: BackgroundJobRow) => {
  ...
  if (row.job_type === "process_paypal_webhook") {
    const {
      processStoredPaypalWebhookEvent,
    } = require("./paypalWebhook.service");
