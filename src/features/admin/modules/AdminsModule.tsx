    ...
    <div className="admin-page-stack">
      <div className="admin-metrics-grid">
        <div className="admin-card" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div><strong style={{ fontSize: 22 }}>{rootCount}</strong><p className="admin-muted">Root account{rootCount === 1 ? '' : 's'}</p></div>
        </div>
        <div className="admin-card" style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <div><strong style={{ fontSize: 22 }}>{adminCount}</strong><p className="admin-muted">Administrator{adminCount === 1 ? '' : 's'}</p></div>
        </div>
      </div>
      ...
      <div className="admin-card" style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 16px' }}>
        ...
        <span className="admin-muted" style={{ fontSize: 13 }}>
          Read-only view. Admin accounts can only be managed by the root account from the Users section.
        </span>
      </div>

      <FilterBar>
        <label className="admin-search-field"><Search size={17} /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search administrators" maxLength={120} /></label>
      </FilterBar>

      {error && <div className="admin-inline-error">{error}</div>}
      {loading ? <Skeleton rows={5} /> : admins.length === 0 ? <EmptyState title="No administrators found" description="No staff accounts match your search." /> : (
        <DataTable
          rows={admins}
          rowKey={(admin) => admin.id_user}
          columns={[
            {
              key: 'admin', label: 'Administrator', render: (admin) => (
                <div className="admin-primary-cell">
                  <strong>
                    {admin.name} {admin.lastname}
                    {admin.id_user === currentId && <span className="admin-pill" style={{ marginLeft: 8, background: 'rgba(16,185,129,0.14)', color: '#047857' }}>You</span>}
                  </strong>
                  <span>{admin.email}</span>
                </div>
              ),
            },
            { key: 'role', label: 'Role', render: (admin) => <RoleBadge rol={admin.rol} /> },
            { key: 'status', label: 'Status', render: (admin) => <StatusBadge status={admin.is_active ? 'active' : 'inactive'} /> },
            { key: 'created', label: 'Joined', render: (admin) => new Date(admin.created_at).toLocaleDateString() },
            { key: 'login', label: 'Last login', render: (admin) => admin.last_login ? new Date(admin.last_login).toLocaleString() : 'Never' },
          ]}
        />
      )}
    </div>
