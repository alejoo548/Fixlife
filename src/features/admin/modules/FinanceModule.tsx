...
export default function FinanceModule() {
  ...
  return <div className="admin-page-stack">
    ...
    {tab==='rules' && <AdminCard><div className="admin-section-heading"><div><p className="admin-section-title">Commission engine</p><p className="admin-muted">Default rate plus existing service, urgency and tier overrides.</p></div><Scale/></div><FormSection title="Default commission"><label className="admin-field"><span>Platform rate (%)</span><AdminNumberInput min={0} max={50} decimals={2} value={commissionRate} onChange={(v)=>setCommissionRate(v)}/></label><div className="admin-kv"><span>Service overrides</span><strong>{commission?.service_overrides?.length||0}</strong></div><div className="admin-kv"><span>Urgency rules</span><strong>{commission?.urgency_adjustments?.length||0}</strong></div><div className="admin-kv"><span>Tier rules</span><strong>{commission?.worker_tier_adjustments?.length||0}</strong></div></FormSection><button className="admin-button" disabled={saving} onClick={saveCommission}>Save commission rate</button></AdminCard>}
