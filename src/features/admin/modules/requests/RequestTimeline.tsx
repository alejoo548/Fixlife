...
export function RequestTimeline({ request }: { request: AdminRequestDetail }) {
  const events = [
    ...
    { label: 'Funds released', date: request.payment?.released_at, detail: request.payment?.released_at ? 'Professional payout released' : 'Not released', icon: CheckCircle2, done: !!request.payment?.released_at },
    { label: 'Customer rated service', date: request.rating?.created_at, detail: request.rating?.comment || 'No rating submitted', icon: Star, done: !!request.rating },
  ];
  return (
