      ...
      >
        <div className="flex items-center justify-between border-b px-6 py-4">
          <div>
            ...
            <h2 className="text-2xl font-black text-slate-950">{t('serviceRequest.history.requestsAndHistory')}</h2>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="flex items-center gap-2 border-b px-6 py-3 bg-slate-50">
          {['all', 'in_progress', 'done', 'cancelled'].map((s) => {
            const label = s === 'all'
              ? t('serviceRequest.history.filters.all')
              : s === 'in_progress'
                ? t('serviceRequest.history.active')
                : s === 'done'
                  ? t('serviceRequest.history.filters.done')
                  : t('serviceRequest.history.filters.cancelled');
          ...
          <div className="ml-auto text-xs text-slate-500">{t('serviceRequest.history.requestsCount', { count: visibleRequests.length })}</div>
        </div>

        <div className="flex flex-1 min-h-0 flex-col lg:flex-row overflow-hidden">
          <div className="w-full lg:w-80 border-r overflow-y-auto p-4 bg-white custom-scrollbar">
            {historyLoading && <div className="p-4 text-sm text-slate-500">{t('serviceRequest.history.loading')}</div>}

            {visibleRequests.length === 0 && !historyLoading && (
              <div className="p-8 text-center text-sm text-slate-500">{t('serviceRequest.history.empty')}</div>
            )}
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <div className="font-black text-slate-950 truncate">{localizeServiceName(req.service_name, currentLanguage)}</div>
          ...
          <div className="flex-1 flex flex-col min-h-0 bg-slate-50 p-4 lg:p-6 overflow-hidden">
            {!trackableRequest && (
              <div className="flex h-full flex-col items-center justify-center text-center">
                ...
                <p className="text-xl font-black text-slate-900">{t('serviceRequest.history.selectRequest')}</p>
                <p className="mt-2 max-w-sm text-sm text-slate-600">
                  {t('serviceRequest.history.selectRequestSubtitle')}
                </p>
                <p className="mt-4 text-xs text-emerald-600 font-medium">{t('serviceRequest.history.completedNote')}</p>
              </div>
            )}
            ...
            {trackableRequest && (
              <div className="flex h-full flex-col">
                <div className="mb-3 flex items-center justify-between">
                  <div>
                    <div className="text-sm font-bold text-slate-500">{t('serviceRequest.history.liveTracking')}</div>
                    <div className="font-black text-xl text-slate-950">{localizeServiceName(trackableRequest.service_name, currentLanguage)}</div>
                  </div>
