    ...
    return (
        <>
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
                <label className="block text-sm font-bold text-gray-700 mb-2">{t('serviceRequest.problem.title')}</label>
                <textarea
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.25 }}>
                ...
                <label className="block cursor-pointer">
                    <div className="w-full rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 hover:bg-slate-100 hover:border-slate-300 transition-all duration-200 p-6 text-center group">
                        ...
                        <div className="text-slate-900 font-bold text-[15px]">{t('serviceRequest.problem.uploadPhotos')}</div>
                        <div className="text-[13px] text-slate-500 mt-1">{t('serviceRequest.problem.imageTypes')}</div>
                    </div>
                    <input
                        type="file"
                        multiple
            ...
            {showBudget && (
                <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
                    <label className="block text-sm font-bold text-slate-900 mb-2">{t('serviceRequest.problem.budget')}</label>
                        ...
                        {nearbyWorkers.slice(0, 5).map((worker) => (
                            <div key={worker.id_worker_profile} className="flex items-center justify-between rounded-lg bg-gray-50 border border-gray-100 px-3 py-2">
                                <div className="min-w-0">
                                    ...
                                    <p className="text-xs text-gray-500 truncate">{worker.bio || t('serviceRequest.problem.availableNow')}</p>
                                </div>
