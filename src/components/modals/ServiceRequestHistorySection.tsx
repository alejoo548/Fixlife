    ...
    return (
        <div className="mt-5 rounded-2xl border border-gray-200 bg-white p-4">
            <div className="flex items-center justify-between gap-2 mb-3">
                <p className="text-xs uppercase tracking-wider font-bold text-gray-500">{t('serviceRequest.history.title')}</p>
                <select
                    value={historyStatus}
                ...
                <div className="mb-3 rounded-[2rem] border border-slate-200/50 bg-white p-4 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                        <div className="min-w-0">
                            ...
                            <p className="mt-1 text-base font-black text-slate-900 truncate">{localizeServiceName(activeTrackedRequest.service_name, currentLanguage)}</p>
                    ...
                    <div className="mt-4 grid grid-cols-3 gap-2">
                        <div className="rounded-xl bg-slate-50 border border-slate-100 p-3">
                            ...
                            <p className="mt-0.5 text-sm font-black text-slate-900">
                                {activeTrackedRequest.status === 'done' ? '0 min' : t('serviceRequest.history.live')}
                            </p>
                        </div>
                        ...
                        <div className="rounded-xl bg-slate-50 border border-slate-100 p-3">
                            ...
                            <p className="mt-0.5 text-sm font-black text-slate-900">{t('serviceRequest.history.onMap')}</p>
                        </div>
                    </div>
                </div>
            )}
