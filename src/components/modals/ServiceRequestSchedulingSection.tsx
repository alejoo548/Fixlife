...
const TIME_PERIODS = [
    { id: 'morning', labelKey: 'morning', start: 5, end: 12 },
    { id: 'afternoon', labelKey: 'afternoon', start: 12, end: 18 },
    { id: 'evening', labelKey: 'evening', start: 18, end: 24 },
    { id: 'overnight', labelKey: 'overnight', start: 0, end: 5 },
] as const;
...
const monthLabel = (date: Date, language: string) =>
    date.toLocaleDateString(language.startsWith('es') ? 'es-SV' : 'en-US', { month: 'long', year: 'numeric' });
    ...
    return (
        <section className="rounded-[26px] border border-slate-200 bg-white p-4 shadow-sm">
            <div className="flex items-center justify-between gap-3">
                <div>
                    <p className="text-[11px] font-black uppercase tracking-[0.18em] text-slate-400">{t('serviceRequest.scheduling.modeLabel')}</p>
                    <h3 className="mt-1 text-lg font-black text-slate-900">{t('serviceRequest.scheduling.title')}</h3>
                    <p className="mt-1 text-xs font-semibold text-slate-500">{t('serviceRequest.scheduling.subtitle')}</p>
                </div>
            ...
            <div className="mt-4 grid grid-cols-1 gap-3 sm:grid-cols-2">
                ...
                >
                    <p className="text-sm font-black text-slate-900">{t('serviceRequest.scheduling.expressTitle')}</p>
                    <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">{t('serviceRequest.scheduling.expressSubtitle')}</p>
                </button>
                ...
                >
                    <p className="text-sm font-black text-slate-900">{t('serviceRequest.scheduling.scheduledTitle')}</p>
                    <p className="mt-1 text-xs font-semibold leading-5 text-slate-500">{t('serviceRequest.scheduling.scheduledSubtitle')}</p>
                </button>
            </div>
                ...
                <div className="mt-4 space-y-3">
                    <div className="overflow-hidden rounded-[22px] border border-slate-200 bg-slate-50">
                        <div className="flex items-center justify-between gap-3 border-b border-slate-200 bg-white px-4 py-3">
                            ...
                            <div className="text-center">
                                <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">{t('serviceRequest.scheduling.date')}</p>
                                <p className="text-sm font-black capitalize text-slate-900">{monthLabel(visibleMonth, i18n.language)}</p>
                            </div>
                        ...
                        <div className="grid grid-cols-7 gap-1 px-3 pt-3 text-center text-[10px] font-black uppercase text-slate-400">
                            {(i18n.language.startsWith('es') ? ['D', 'L', 'M', 'M', 'J', 'V', 'S'] : ['S', 'M', 'T', 'W', 'T', 'F', 'S']).map((day, index) => (
                    ...
                    <div className="rounded-[22px] border border-slate-200 bg-white p-3">
                        <div className="mb-3 flex items-center justify-between gap-3">
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">{t('serviceRequest.scheduling.estimatedDuration')}</p>
                                <p className="mt-1 text-xs font-bold text-slate-600">{t('serviceRequest.scheduling.estimatedDurationSubtitle')}</p>
                            </div>
                    ...
                    <div className="rounded-[22px] border border-slate-200 bg-white p-3">
                        <div className="mb-3 flex items-center justify-between gap-3">
                            <div>
                                <p className="text-[10px] font-black uppercase tracking-[0.18em] text-slate-400">{t('serviceRequest.scheduling.startTime')}</p>
                        ...
                        <div className="mb-3 grid grid-cols-4 gap-1 rounded-xl bg-slate-100 p-1">
                            {TIME_PERIODS.map((period) => (
                                ...
                                >
                                    {t(`serviceRequest.scheduling.periods.${period.labelKey}`)}
                                </button>
                            ))}
                        </div>
