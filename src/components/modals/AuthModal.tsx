    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      {isDesktop ? (
        <div className="relative z-[101] w-full max-w-[850px] h-[520px] bg-white rounded-3xl shadow-2xl overflow-hidden text-gray-900 ring-1 ring-gray-200 animate-zoom-in">
          ...
          >
            <h2 className="text-3xl font-bold mb-4 text-bird-blue">{t('auth.createAccountTitle')}</h2>
            <p className="text-xs text-gray-500 mb-4">{t('auth.registerWithEmail')}</p>
            <form className="w-full flex flex-col gap-2.5" onSubmit={handleSignup}>
              <div className="grid grid-cols-2 gap-2.5">
                <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                  <input type="text" name="name" maxLength={50} value={formData.name} onChange={handleChange} placeholder={t('auth.fields.firstName')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                </div>
                <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                  <input type="text" name="lastname" maxLength={50} value={formData.lastname} onChange={handleChange} placeholder={t('auth.fields.lastName')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                </div>
              </div>

              <div className="grid grid-cols-2 gap-2.5">
                <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                  <input type="text" name="username" maxLength={30} value={formData.username} onChange={handleChange} placeholder={t('auth.fields.username')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                </div>
                <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                  <input type="tel" name="phone_number" maxLength={15} value={formData.phone_number} onChange={handleChange} placeholder={t('auth.fields.phone')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                </div>
              </div>

              <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                <input type="email" name="email" maxLength={100} value={formData.email} onChange={handleChange} placeholder={t('auth.fields.email')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
              </div>

              <div className="grid grid-cols-2 gap-2.5">
                <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                  <PasswordInput maxLength={128}  name="password" value={formData.password} onChange={handleChange} placeholder={t('auth.fields.password')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                </div>
                <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                  <PasswordInput maxLength={128}  name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} placeholder={t('auth.fields.confirm')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                </div>
              </div>
          ...
          <div
            className={`absolute top-0 left-0 w-1/2 h-full z-10 flex flex-col items-center justify-center px-14 ${transitionClass}
             ${!isSignup ? 'translate-x-0 opacity-100 z-10' : '-translate-x-[20%] opacity-0 z-0'}`}
          >
            <h2 className="text-3xl font-bold mb-6 text-bird-blue">{t('auth.signInTitle')}</h2>
            <div className="flex flex-col gap-3 w-full mb-6">
              <GoogleSignInButton onCredential={handleGoogleSignin} />
            </div>

            <p className="text-xs text-gray-500 mb-4">{t('auth.signInWithEmail')}</p>
            <form className="w-full flex flex-col gap-3" onSubmit={handleSignin}>
              ...
              <button type="button" onClick={() => setView('forgot')} className="text-xs text-gray-500 hover:text-bird-blue transition-colors self-end my-1"> {t('auth.forgotPassword')} </button>
          ...
          <div className="relative z-10 flex flex-col h-full p-6 overflow-y-auto custom-scrollbar">
            ...
            <div className="flex-1 flex flex-col justify-center transition-all duration-500">
              ...
              <div className="relative flex py-2 items-center mb-6">
                ...
                <span className="flex-shrink-0 mx-4 text-xs text-gray-500">{t('auth.continueWithEmail')}</span>
                <div className="flex-grow border-t border-gray-200"></div>
              </div>
              ...
              <form className="flex flex-col gap-3" onSubmit={isSignup ? handleSignup : handleSignin}>
                  ...
                  <div className="animate-fade-in-up space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                        <input type="text" name="name" maxLength={50} value={formData.name} onChange={handleChange} placeholder={t('auth.fields.name')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                      </div>
                      <div className="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                        <input type="text" name="lastname" maxLength={50} value={formData.lastname} onChange={handleChange} placeholder={t('auth.fields.surname')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                      </div>
                    </div>
                    <div className="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                      <input type="text" name="username" maxLength={30} value={formData.username} onChange={handleChange} placeholder={t('auth.fields.username')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                    </div>
                    <div className="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                      <input type="tel" name="phone_number" maxLength={15} value={formData.phone_number} onChange={handleChange} placeholder={t('auth.fields.phoneNumber')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                    </div>
                  </div>
                )}

                <div className="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                  <input type="email" name="email" maxLength={100} value={formData.email} onChange={handleChange} placeholder={t('auth.fields.emailAddress')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                </div>

                <div className="grid grid-cols-1 gap-3">
                  <div className="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-blue/50 transition-colors">
                    <PasswordInput maxLength={128}  name="password" value={formData.password} onChange={handleChange} placeholder={t('auth.fields.password')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                  </div>
                  {isSignup && (
                    <div className="bg-gray-50 rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-blue/50 transition-colors animate-fade-in-up">
                      <PasswordInput maxLength={128}  name="confirmPassword" value={formData.confirmPassword} onChange={handleChange} placeholder={t('auth.fields.confirmPassword')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                    </div>
                  )}
                </div>
                ...
                <button className={`mt-4 w-full py-4 rounded-xl font-bold text-sm tracking-wide shadow-lg active:scale-[0.98] transition-all duration-300 ${isSignup ? 'bg-gradient-to-r from-bird-yellow to-bird-orange text-gray-900 shadow-bird-yellow/20' : 'bg-gradient-to-r from-bird-blue to-bird-darkBlue text-white shadow-bird-blue/20'}`}>
                  {isSignup ? t('auth.actions.createAccount') : t('auth.actions.signIn')}
                </button>
              </form>
            </div>
          </div>
