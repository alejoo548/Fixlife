...
export const WorkerAuthModal: React.FC<WorkerAuthModalProps> = ({ isOpen, onClose, mode: initialMode, onSuccess }) => {
  ...
  const handleSpecialtiesSubmit = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        void showSweetToast({ tone: 'error', message: data.error || t('workerAuth.messages.registrationFailed') });
        setLoading(false);
        return;
      }
  ...
  const handleVerifySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        void showSweetToast({ tone: 'error', message: data.error || t('workerAuth.messages.verificationFailed') });
        setLoading(false);
        return;
      }
  ...
  const handleSigninSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        void showSweetToast({ tone: 'error', message: data.error || t('workerAuth.messages.loginFailed') });
        setLoading(false);
        return;
      }
  ...
  const handleUploadDocuments = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        void showSweetToast({ tone: 'error', message: data.error || t('workerAuth.messages.uploadError') });
        setLoading(false);
        return;
      }
      ...
      <div className="hidden md:flex relative w-full max-w-[850px] h-[580px] bg-white rounded-3xl shadow-2xl overflow-hidden text-gray-900 ring-1 ring-gray-200 animate-zoom-in">
          ...
          <AnimatePresence mode="wait">
            {view === 'signup' && (
              ...
              >
                <div className="flex flex-col items-center text-center w-full">
                  <h2 className="text-3xl font-bold mb-2 text-bird-orange">{t('workerAuth.joinTitle')}</h2>
                  <p className="text-xs text-gray-500 mb-4">{t('workerAuth.joinDescription')}</p>
                  ...
                  <form className="w-full flex flex-col gap-2.5" onSubmit={handleSignupSubmit}>
                    <div className="grid grid-cols-2 gap-2.5">
                      <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-orange/50 transition-colors">
                        <input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} type="text" maxLength={50} placeholder={t('workerAuth.fields.firstName')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                      </div>
                      <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-orange/50 transition-colors">
                        <input required value={formData.lastname} onChange={e => setFormData({...formData, lastname: e.target.value})} type="text" maxLength={50} placeholder={t('workerAuth.fields.lastName')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                      </div>
                    </div>

                    <div className="grid grid-cols-2 gap-2.5">
                      <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-orange/50 transition-colors">
                        <input value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} type="text" maxLength={30} placeholder={t('workerAuth.fields.usernameOptional')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                      </div>
                      <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-orange/50 transition-colors">
                        <input required value={formData.phone_number} onChange={e => setFormData({...formData, phone_number: e.target.value})} type="tel" maxLength={15} placeholder={t('workerAuth.fields.phoneNumber')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                      </div>
                    </div>

                    <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-orange/50 transition-colors">
                      <input required value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} type="email" maxLength={100} placeholder={t('workerAuth.fields.emailAddress')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                    </div>

                    <div className="grid grid-cols-2 gap-2.5">
                      <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-orange/50 transition-colors">
                        <input required value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} type="password" maxLength={128} placeholder={t('workerAuth.fields.password')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                      </div>
                      <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-orange/50 transition-colors">
                        <input required value={formData.confirmPassword} onChange={e => setFormData({...formData, confirmPassword: e.target.value})} type="password" maxLength={128} placeholder={t('workerAuth.fields.confirmPassword')} className="w-full bg-transparent px-3 py-2 text-sm text-gray-900 outline-none placeholder-gray-500" />
                      </div>
                    </div>
            ...
            {view === 'specialties' && (
              ...
              >
                ...
                <h2 className="text-3xl font-bold mb-2 text-bird-orange">{t('workerAuth.specialtiesTitle')}</h2>
                <p className="text-sm text-gray-600 mb-5">{t('workerAuth.specialtiesDescription')}</p>
            ...
            {view === 'verify' && (
              ...
              >
                ...
                <h2 className="text-3xl font-bold mb-2 text-bird-orange">{t('workerAuth.verifyTitle')}</h2>
                <p className="text-sm text-gray-600 mb-6">
                  {t('workerAuth.verifyDescription')} <br/><span className="font-semibold text-gray-900">{registeredEmail}</span>
                </p>
                    ...
                    onClick={async () => {
                      try {
                        ...
                        if (res.ok) {
                          ...
                          void showSweetToast({ tone: 'error', message: data.error || t('workerAuth.messages.resendFailed') });
                        }
                      } catch {
                        void showSweetToast({ tone: 'error', message: t('workerAuth.messages.connectionError') });
                      }
            ...
            {view === 'upload' && (
              ...
              >
                ...
                <h2 className="text-3xl font-bold mb-2 text-gray-900">{t('workerAuth.uploadTitle')}</h2>
                ...
                <label className="w-full border-2 border-dashed border-gray-300 rounded-2xl p-6 mb-3 bg-gray-50 flex flex-col items-center justify-center hover:bg-gray-100 transition-colors cursor-pointer group">
                  ...
                  <span className="text-xs text-gray-500 mt-1">{t('workerAuth.fileTypesHint')}</span>
        ...
        >
          ...
          <form className="w-full flex flex-col gap-4" onSubmit={handleSigninSubmit}>
            ...
            <div className="bg-gray-50 rounded-lg p-1 border border-gray-200 focus-within:border-bird-orange/50 transition-colors">
              <input required value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} type="password" maxLength={128} placeholder={t('workerAuth.fields.password')} className="w-full bg-transparent px-3 py-3 text-sm text-gray-900 outline-none placeholder-gray-500" />
            </div>
            ...
            <AnimatePresence mode="wait">
              {view === 'signup' && (
                ...
                >
                  <div className="text-center mb-6">
                    <h2 className="text-2xl font-bold text-gray-900 mb-2">{t('workerAuth.joinTitle')}</h2>
                    <p className="text-xs text-gray-600 px-4">{t('workerAuth.joinDescription')}</p>
                  </div>
                  <form className="flex flex-col gap-3" onSubmit={handleSignupSubmit}>
                    <div className="grid grid-cols-2 gap-3">
                      <div className="bg-white rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-orange/50 transition-colors shadow-sm">
                        <input required value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} type="text" maxLength={50} placeholder={t('workerAuth.fields.firstName')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                      </div>
                      <div className="bg-white rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-orange/50 transition-colors shadow-sm">
                        <input required value={formData.lastname} onChange={e => setFormData({...formData, lastname: e.target.value})} type="text" maxLength={50} placeholder={t('workerAuth.fields.lastName')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                      </div>
                    </div>
                    <div className="bg-white rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-orange/50 transition-colors shadow-sm">
                      <input value={formData.username} onChange={e => setFormData({...formData, username: e.target.value})} type="text" maxLength={30} placeholder={t('workerAuth.fields.usernameOptional')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                    </div>
                    <div className="bg-white rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-orange/50 transition-colors shadow-sm">
                      <input required value={formData.phone_number} onChange={e => setFormData({...formData, phone_number: e.target.value})} type="tel" maxLength={15} placeholder={t('workerAuth.fields.phoneNumber')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                    </div>
                    <div className="bg-white rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-orange/50 transition-colors shadow-sm">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      <div className="bg-white rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-orange/50 transition-colors shadow-sm">
                        <input required value={formData.password} onChange={e => setFormData({...formData, password: e.target.value})} type="password" maxLength={128} placeholder={t('workerAuth.fields.password')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                      </div>
                      <div className="bg-white rounded-xl px-4 py-3 border border-gray-200 focus-within:border-bird-orange/50 transition-colors shadow-sm animate-fade-in-up">
                        <input required value={formData.confirmPassword} onChange={e => setFormData({...formData, confirmPassword: e.target.value})} type="password" maxLength={128} placeholder={t('workerAuth.fields.confirmPassword')} className="w-full bg-transparent text-sm text-gray-900 outline-none placeholder-gray-500" />
                      </div>
                    </div>
              ...
              {view === 'specialties' && (
                ...
                >
                  ...
                  <p className="text-sm text-gray-600 mb-5 px-2">{t('workerAuth.specialtiesDescription')}</p>
