    ...
    >
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="shrink-0 text-right">
          <p className="text-[10px] font-black uppercase text-slate-400 dark:text-slate-400">{t('workerDashboard.requestCard.offer')}</p>
      ...
      <div className="mt-3 flex flex-wrap items-center gap-2">
        <span className={`rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${
          ...
          {scheduled ? t('workerDashboard.requestCard.scheduledVisit') : t('workerDashboard.requestCard.expressVisit')}
        </span>
        <span className={`rounded-full border px-2.5 py-1 text-[10px] font-black uppercase tracking-[0.12em] ${
          isFastMatch
          ...
          {isFastMatch ? t('workerDashboard.requestCard.fastMatch') : t('workerDashboard.requestCard.reviewAndChoose')}
        </span>
      </div>
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <div className="flex min-w-0 items-center gap-2 rounded-xl bg-slate-50 dark:bg-slate-800/80 px-3 py-2">
          ...
          <span className="truncate text-[11px] font-bold text-slate-600 dark:text-slate-300">
            {request.distance_km != null ? t('workerDashboard.requestCard.kmAway', { km: request.distance_km.toFixed(1) }) : t('workerDashboard.requestCard.distancePending')}
          </span>
        </div>
        <div className="flex min-w-0 items-center gap-2 rounded-xl bg-slate-50 dark:bg-slate-800/80 px-3 py-2">
          {scheduled ? (
          ...
          <span className="truncate text-[11px] font-bold text-slate-600 dark:text-slate-300">
            {scheduled ? scheduledWindow : t('workerDashboard.requestCard.expressVisit')}
          </span>
        </div>
      </div>
        ...
        {statusFilter === 'new' ? (
          <>
            <button
              ...
              title={t('workerDashboard.requestCard.rejectRequest')}
            ...
            >
              {t('workerDashboard.requestCard.counter')}
            </button>
            <button
              onClick={(event) => {
                event.stopPropagation();
            ...
            >
              ...
              {t('workerDashboard.requestCard.accept')}
            </button>
          </>
          ...
          >
            {t('workerDashboard.requestCard.waitingForClientApproval')}
          </button>
          ...
          >
            ...
            {t('workerDashboard.requestCard.openRoute')}
          </button>
          ...
          >
            ...
            {t('workerDashboard.requestCard.openCurrentJob')}
          </button>
          ...
          >
            {t('workerDashboard.requestCard.waitingForClientConfirmation')}
          </button>
        ) : (
          <button
            onClick={(event) => {
      ...
      {statusFilter === 'new' && actionLockedByActiveJob && (
        <p className="mt-2 text-[11px] font-semibold text-amber-700">
          {t('workerDashboard.requestCard.finishCurrentJobFirst')}
        </p>
      )}
      {request.proposed_budget != null && (
        <p className="text-xs text-amber-700 font-semibold mt-2">
          {t('workerDashboard.requestCard.counterOfferLabel', { amount: request.proposed_budget.toFixed(2) })}
