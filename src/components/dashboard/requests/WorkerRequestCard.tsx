  ...
  return (
    <div
      ...
      className={`group w-full rounded-2xl border p-4 text-left transition-all ${
        selected
          ? 'border-bird-blue bg-sky-50/80 shadow-[0_14px_35px_rgba(0,144,255,0.12)]'
          : 'border-slate-200 bg-white shadow-[0_8px_24px_rgba(15,23,42,0.04)] hover:-translate-y-0.5 hover:border-sky-200 hover:shadow-[0_14px_32px_rgba(15,23,42,0.08)]'
      }`}
    >
      <div className="flex items-start justify-between gap-3">
        <div className="flex min-w-0 items-start gap-3">
          <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-sm font-black ${
            selected ? 'bg-bird-blue text-white' : 'bg-slate-100 text-slate-700'
          }`}>
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-[11px] font-bold text-slate-400">Request #{request.id_request}</p>
          </div>
        </div>
        <div className="shrink-0 text-right">
          <p className="text-[10px] font-black uppercase text-slate-400">Offer</p>
          <p className="text-lg font-black text-slate-950">${request.budget.toFixed(0)}</p>
        </div>
      </div>
      ...
      <div className="mt-2 flex items-center justify-between gap-3">
        <p className="min-w-0 truncate text-xs font-semibold text-slate-500">{request.location_text}</p>
        {statusFilter !== 'new' && (
          <span className="shrink-0 rounded-full border border-slate-200 bg-white px-2.5 py-1 text-[9px] font-black uppercase text-slate-600">
        ...
        {statusFilter === 'new' ? (
          <>
            ...
            <button
              ...
              disabled={busy || actionLockedByActiveJob}
              className="h-10 flex-1 rounded-xl border border-amber-200 bg-amber-50 px-3 text-xs font-black text-amber-800 transition hover:bg-amber-100 disabled:opacity-40"
            >
              Counter
            </button>
            <button
          ...
          <button
            type="button"
            disabled
            className="h-10 w-full rounded-xl bg-sky-50 text-xs font-black text-sky-700"
          >
          ...
          <button
            ...
            className="flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-slate-950 text-xs font-black text-white transition hover:bg-slate-800 disabled:opacity-50"
            disabled={busy}
          >
          ...
          <button
            ...
            disabled={busy}
            className="flex h-10 w-full items-center justify-center gap-2 rounded-xl bg-slate-950 text-xs font-black text-white disabled:opacity-50"
          >
          ...
          <button
            type="button"
            disabled
            className="h-10 w-full rounded-xl bg-violet-50 text-xs font-black text-violet-700"
          >
