...
export const StepsSection: React.FC = () => {
  ...
  const translatedSteps = t('steps.items', { returnObjects: true }) as Array<{
    id: string;
    title: string;
    desc: string;
  }>;
  ...
  return (
    <div className="w-full relative">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-8 relative">
        ...
        {steps.map((step, i) => {
          const translatedStep = translatedSteps[i];
          ...
          >
            ...
            >
              ...
              <div className={`absolute -top-3 -right-3 w-9 h-9 rounded-full bg-gradient-to-br ${step.gradient} flex items-center justify-center shadow-lg border-4 border-white`}>
                <span className="text-white font-black text-sm">{translatedStep?.id || step.id}</span>
              </div>
