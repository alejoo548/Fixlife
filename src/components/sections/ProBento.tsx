...
const StatIcon: React.FC<{ type: string }> = ({ type }) => {
  ...
  if (type === '5.0' || type === '4.9' || type === '4.8') {
    return <span className="text-bird-yellow font-bold text-lg drop-shadow-sm">★ {type}</span>;
  }

  if (type === 'verified') {
    return <span className="text-blue-500 bg-blue-50 px-2.5 py-1 rounded-lg text-[10px] font-bold border border-blue-200 tracking-wider">{t('pros.statLabels.verified')}</span>;
  }

  if (type === 'certified') {
    return <span className="text-emerald-500 bg-emerald-50 px-2.5 py-1 rounded-lg text-[10px] font-bold border border-emerald-200 tracking-wider">{t('pros.statLabels.certified')}</span>;
  }

  if (type === 'expert') {
    return <span className="text-purple-500 bg-purple-50 px-2.5 py-1 rounded-lg text-[10px] font-bold border border-purple-200 tracking-wider">{t('pros.statLabels.expert')}</span>;
  }

  if (type === 'top-rated') {
    return <span className="text-orange-500 bg-orange-50 px-2.5 py-1 rounded-lg text-[10px] font-bold border border-orange-200 tracking-wider">{t('pros.statLabels.topRated')}</span>;
  }

  if (type === 'fast') {
    return <span className="text-cyan-500 bg-cyan-50 px-2.5 py-1 rounded-lg text-[10px] font-bold border border-cyan-200 tracking-wider">{t('pros.statLabels.fast')}</span>;
  }

  if (type === 'insured') {
    return <span className="text-indigo-500 bg-indigo-50 px-2.5 py-1 rounded-lg text-[10px] font-bold border border-indigo-200 tracking-wider">{t('pros.statLabels.insured')}</span>;
  }

  return null;
};

export const ProBento: React.FC<ProBentoProps> = ({ onOpenWorkerAuth }) => {
  const { t } = useTranslation();
  const [activeProfile, setActiveProfile] = useState(0);

  const translatedProfiles = t('pros.profiles', { returnObjects: true }) as Array<{
    role: string;
    bio: string;
    stats: string[];
  }>;

  const translatedBenefits = t('pros.benefits', { returnObjects: true }) as string[];
  ...
  const translatedProfile = translatedProfiles[activeProfile];
        ...
        <div className="flex-1 max-w-xl text-center lg:text-left order-2 lg:order-1">
          ...
          <div className="mb-8 space-y-3">
            {[
              ...
              <div key={benefit.icon} className="flex items-center gap-3 text-left">
                ...
                <span className="text-gray-700 font-medium">{translatedBenefits[i]}</span>
              </div>
            ))}
          </div>
              ...
              >
                ...
                >
                  <div className="relative flex-shrink-0 mb-6 sm:mb-0">
                    ...
                    >
                      <span className="text-[10px] text-white/80 uppercase tracking-widest font-bold">{t('pros.statLabels.jobs')}</span>
                  ...
                  <div className="flex-1 flex flex-col items-center sm:items-start text-center sm:text-left sm:pl-10 pt-4 sm:pt-0">
                    ...
                    >
                      {translatedProfile?.role || profile.role}
                    </motion.p>
                    ...
                    >
                      {translatedProfile?.bio || profile.bio}
                    </motion.p>
                    ...
                    >
                      {(translatedProfile?.stats || profile.stats).map((stat, i) => (
                        <motion.div
                          key={`${profile.id}-${stat}-${i}`}
              ...
              {PRO_PROFILES.map((_, index) => (
                <button
                  ...
                  className={`h-2 rounded-full transition-all duration-300 ${
                    index === activeProfile ? 'w-8 bg-bird-blue' : 'w-2 bg-gray-300 hover:bg-gray-400'
                  }`}
